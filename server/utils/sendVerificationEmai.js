...
const sendVerificationEmail = (user) => {
  ...
  const mailOptions = {
    from: '"ArtOa ?" <connect@artoa.io>', // sender address
    ...
    <body style="margin: 0; padding: 0; background-color: #333; background-image: linear-gradient(to bottom, #333, #000); color: #fff; border-radius: 10px;">
      <div style="max-width: 600px; margin: 20px auto; padding: 20px; padding-top: 30px; padding-bottom:30px;">
        ...
        <a href="${process.env.CLIENT_URL}/auth/verify-email?emailToken=${user.emailToken}" style="display: inline-block; padding: 10px 20px; background: linear-gradient(to bottom, #ff9f45, #ff784f); color: #fff; text-decoration: none; border-radius: 5px;">Verify your email</a>
      </div>
    </body>
